...
function loadMainScript()
    ...
    -- AUTO DUPE IMPLEMENTATION WITH 0.1s KICK PRECISION
    local autoDupeActive = false

    CreateToggle(DupePage, "Auto Dupe (Kick Trigger)", false, function(active)
        autoDupeActive = active
        local connection
        
        if active then
            local function triggerAutoDupe()
                if not autoDupeActive or not env.__NullTimeActive then return end
                
                -- Precisión milimétrica exacta de 0.1 segundos post-kick detectado
            ...
            -- Interceptar errores de desconexión o Prompt de Kick en pantalla
            connection = S.LogService.MessageOut:Connect(function(message, messageType)
                if messageType == Enum.MessageType.MessageError or message:lower():find("kick") or message:lower():find("disconnected") then
                    triggerAutoDupe()
                end
            end)
    ...
    local AntiAfkBtn, setAntiAfk = CreateToggle(FarmPage, "Anti-AFK", false, function(active)
    ...
    local AutoToolBtn = CreateToggle(FarmPage, "Auto-Swing Tool", false, function(active)
    ...
    local AutoPromptBtn = CreateToggle(FarmPage, "Auto-Prompt (Ultra-Fast)", false, function(active)
    ...
    local SpeedBtn = CreateToggle(MovementPage, "Speed Boost (60)", false, function(active)
    ...
    local JumpBtn = CreateToggle(MovementPage, "High Jump (100)", false, function(active)
    ...
    local InstaPickBtn = CreateToggle(MovementPage, "Insta Pick Up", false, function(active)
    ...
    local NoclipBtn = CreateToggle(MovementPage, "Noclip", false, function(active)
    ...
    local EspBtn = CreateToggle(ESPPage, "Player ESP", false, function(active)
        espEnabled = active
        if espEnabled then
    ...
    local BoulderEspBtn = CreateToggle(BoulderPage, "Boulder ESP", false, function(active)
    ...
    local AutoFarmBtn = CreateToggle(BoulderPage, "Auto-Farm Boulders", false, function(active)
